...
def get_rewards_by_category():
    ...
    conn = get_db()
    rows = conn.execute(
        "SELECT rewards_json FROM videos WHERE rewards_json != '[]'"
    ).fetchall()
    conn.close()
    ...
    conn.close()
    cache_map = {(r['name'], r['category']): r['wowhead_icon_url'] for r in cached}

    # Assign icons from cache or generate fallback URLs
    from utils.wowhead_image import FALLBACK_ICONS, WOWHEAD_ICON_BASE
    for key, val in reward_lookup.items():
        if key in cache_map and cache_map[key]:
            val['wowhead_icon_url'] = cache_map[key]
        else:
            cat = val['category']
            fallback = FALLBACK_ICONS.get(cat, 'inv_misc_questionmark')
            val['wowhead_icon_url'] = f"{WOWHEAD_ICON_BASE}/{fallback}.jpg"
