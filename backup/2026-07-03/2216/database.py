...
def get_rewards_by_category():
    ...
    # Load cached icons and time-limited status
    conn = get_db()
    cached = conn.execute('SELECT name, category, wowhead_icon_url, is_time_limited, time_limited_note FROM reward_cache').fetchall()
    conn.close()
    cache_map = {(r['category'], r['name']): r['wowhead_icon_url'] for r in cached}
    time_limited_map = {(r['category'], r['name']): {'is_time_limited': bool(r['is_time_limited']), 'time_limited_note': r['time_limited_note']} for r in cached}
    ...
    for key, val in reward_lookup.items():
        ...
        # Attach time-limited status
        if key in time_limited_map:
            val['is_time_limited'] = time_limited_map[key]['is_time_limited']
            val['time_limited_note'] = time_limited_map[key]['time_limited_note']
        else:
            val['is_time_limited'] = False
            val['time_limited_note'] = ''
