...
function initNavIndicator() {
    ...
    // Position on initial active tab (no animation)
    const activeLink = nav.querySelector('a.active') || links[0];
    if (activeLink) {
        // Temporarily disable transitions, position, force layout, then re-enable
        indicator.style.transition = 'none';
        positionIndicator(activeLink);
        // Force browser to apply the non-animated position immediately
