...
def _wowhead_search(name, search_type='item'):
    ...
    params = urllib.parse.urlencode({'q': name, 'type': search_type})
    ...
    # Extract the first result's ID and icon
    # Pattern 1: data-id="12345" ... data-icon="icon_name"
    id_match = re.search(r'data-id="(\d+)"', html)
    icon_match = re.search(r'data-icon="([^"]+)"', html)
    if id_match:
        return {
            'wowhead_id': int(id_match.group(1)),
